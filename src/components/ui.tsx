...
export function CheckCircle({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label ?? (on ? 'Mark unread' : 'Mark read')}
      onClick={(e) => {
        e.preventDefault()
        e.stopPropagation()
        if (!disabled && onClick) {
...
export function AuthorChip({ name, to }: { name: string; to?: string }) {
  const inner = (
    <>
      <span className="authorchip-name">{name}</span>
      <IconChevronRight size={12} strokeWidth={3} />
    </>
  )
  if (to) {
    return (
      <Link className="authorchip" to={to}>
        {inner}
      </Link>
    )
  }
  return <span className="authorchip">{inner}</span>
}

/** The person's own 1–5 rating. Tapping the current value clears it. */
  ...
  return (
    <div className="stars" role="group" aria-label="Your rating">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <IconStar size={22} strokeWidth={1.8} />
        </button>
      ))}
    </div>
  )
