 ...
 * personal data: no search terms, no titles, no identifiers. These counters are
 * never uploaded; the hosted build reports only a cookieless screen-view count
 * (see Analytics).
 */
...
async function readEnvironment(): Promise<Environment> {
  ...
  return {
    ...
    libraryKb: kb(localStorage.getItem('bookstable-library-v1') ?? ''),
    cacheKb: kb(localStorage.getItem('bookstable-cache-v1') ?? ''),
...
export default function InsightsPage() {
  ...
  const books = useLibrary((x) => x.books)
  const entries = useBookCache((x) => x.entries)
  ...
  const library = buildStats(books, entries)
  const dropped = Object.values(books).filter((t) => t.status === 'dropped').length
  ...
  const sources = (['openlibrary'] as SourceName[])
    .map((name) => [name.toUpperCase(), s.sourceHits[name] ?? 0] as [string, number])
    .filter(([, v]) => v > 0)
  const errors = (['openlibrary'] as SourceName[])
  ...
  return (
    <div className="page insights">
      ...
      <p className="chips-hint">
        ...
        {library.finished} finished · {library.want} waiting ·{' '}
        {Object.keys(entries).length} book{Object.keys(entries).length === 1 ? '' : 's'} cached
      </p>
