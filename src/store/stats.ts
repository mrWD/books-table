...
export type SourceName = 'openlibrary'
...
interface StatsState {
  ...
  searches: number
  /** Which source actually answered. One source today; the field outlives that. */
  ...
  recordCheckIn: () => void
  recordFinished: () => void
  recordRecommendations: (count: number) => void
  reset: () => void
}
...
export const useStats = create<StatsState>()(
  persist(
    (set) => ({
      ...
      recordCheckIn: () => set((s) => ({ checkIns: s.checkIns + 1 })),

      recordFinished: () => set((s) => ({ booksFinished: s.booksFinished + 1 })),

      recordRecommendations: (count) =>
        set((s) => ({ recommendationsShown: s.recommendationsShown + count })),

      reset: () => set({ ...EMPTY }),
    }),
    { name: 'bookstable-stats-v1', version: 1 },
  ),
)
...
export function beginSessionOnce(): void {
  try {
    if (sessionStorage.getItem('bt-session') === '1') return
    sessionStorage.setItem('bt-session', '1')
  } catch {
